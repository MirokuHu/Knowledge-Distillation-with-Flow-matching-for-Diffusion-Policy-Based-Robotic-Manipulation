...
class TeacherDataset(Dataset):
    ...
    def __init__(self, path: str, action_key: str = "teacher_action"):
        ...
        available_keys = list(self.data[0].keys())
        if "obs" not in available_keys:
            raise KeyError(f"'obs' not found in sample. Available keys: {available_keys}")

        if action_key not in available_keys:
            raise KeyError(
                f"action_key='{action_key}' not found in sample. "
